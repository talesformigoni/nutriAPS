...
from nutri_pdf import gerar_pdf_paciente, gerar_pdf_gestante 
...
with main_col_2:
    ...
    if perfil_clinico == "👤 População Geral":
        ...
        with st.container():
            ...
            with cc1:
                is_dmg = st.checkbox("🩸 Diabetes Gestacional (DMG)")
            with cc2:
                is_has = st.checkbox("🫀 Hipertensão Arterial (HAS)")
        ...
        if altura > 0 and peso_atual > 0:
            ...
            st.session_state["dados_paciente"] = {
                ...
                "is_dmg": is_dmg, # Adicionado
                "is_has": is_has  # Adicionado
            }
            # --- BOTÃO DE DOWNLOAD PDF GESTANTE ---
            st.markdown("<br>", unsafe_allow_html=True) # Dá um espacinho para não colar no gráfico
